...
try {
  ...
  ok('blank value ignored', getPrefs('u@x.com').lastTarget === 'prod' && getPrefs('u@x.com').lastType === 'ApexClass');

  // partial merge keeps untouched keys
  setPrefs('u@x.com', { lastType: 'CustomField' });
  ok('merge keeps other keys', getPrefs('u@x.com').lastTarget === 'prod' && getPrefs('u@x.com').lastType === 'CustomField');

  // per-org isolation
  setPrefs('other@y.com', { lastTarget: 'sandbox' });
  ok('orgs isolated', getPrefs('u@x.com').lastTarget === 'prod' && getPrefs('other@y.com').lastTarget === 'sandbox');
