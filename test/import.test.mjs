...
ok(
    'parses explicit members',
    keys.includes('ApexClass:A') &&
        keys.includes('ApexClass:B') &&
        keys.includes('CustomField:Account.Region__c'),
);
ok('dedupes repeated member', keys.filter((k) => k === 'ApexClass:A').length === 1);
ok(
    'reports wildcard types (not as entries)',
    parsed.wildcards.includes('CustomObject') && !keys.some((k) => k.includes('CustomObject')),
);
...
try {
    ...
    ok(
        'importPackage extracts package.xml from a .zip',
        fromZip.entries.length === 3 && fromZip.entries.some((e) => e.fullName === 'Account.Region__c'),
    );

    let threw = false;
    try {
        await importPackage(path.join(dir, 'nope.txt'));
    } catch {
        threw = true;
    }
