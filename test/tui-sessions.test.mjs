...
const SAVED = [
    {
        label: 'last deploy',
        entries: [
            { type: 'ApexClass', fullName: 'LeadService' },
...
const timer = setTimeout(() => {
    console.log('SESSIONS FAIL: timed out');
    process.exit(2);
}, 9000);
...
console.log(
    `after load: selected=${selectionCount(store)} target=${store.targetOrg} action=${result?.action}`,
);
