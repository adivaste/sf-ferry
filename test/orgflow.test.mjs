...
const ra = buildRetrieveArgs({
    manifestPath: 'manifest/package.xml',
    sourceOrg: 'uat',
    metadataDir: '.ferry-retrieve',
});
ok(
    'retrieve uses --target-metadata-dir',
    ra.includes('--target-metadata-dir') && ra.includes('.ferry-retrieve'),
);
ok('retrieve does NOT use --output-dir', !ra.includes('--output-dir'));
ok(
    'retrieve passes manifest + source org',
    ra.includes('manifest/package.xml') && ra[ra.indexOf('--target-org') + 1] === 'uat',
);

// Deploy must use --metadata-dir <zip> and NOT --single-package (zip has an
// unpackaged/ wrapper) and NOT --source-dir.
const da = buildOrgDeployArgs({
    zipPath: `.ferry-retrieve/${RETRIEVE_ZIP}`,
    targetOrg: 'prod',
    testLevel: 'RunLocalTests',
});
ok(
    'deploy uses --metadata-dir zip',
    da.includes('--metadata-dir') && da.includes(`.ferry-retrieve/${RETRIEVE_ZIP}`),
);
...
ok(
    'RunSpecifiedTests passes --tests',
    va.filter((x) => x === '--tests').length === 2 && va.includes('T1') && va.includes('T2'),
);

// validate + NoTestRun is invalid for `deploy validate`; fall back to a
// check-only `deploy start --dry-run`.
const na = buildOrgDeployArgs({
    zipPath: 'z.zip',
    targetOrg: 'prod',
    testLevel: 'NoTestRun',
    validate: true,
...
ok(
    'NoTestRun validate -> start --dry-run',
    na.includes('start') && !na.includes('validate') && na.includes('--dry-run'),
);
const nd = buildOrgDeployArgs({
    zipPath: 'z.zip',
    targetOrg: 'prod',
    testLevel: 'NoTestRun',
    validate: false,
...
const a = manifestSignature([
    { type: 'ApexClass', fullName: 'A' },
    { type: 'ApexClass', fullName: 'B' },
]);
const b = manifestSignature([
    { type: 'ApexClass', fullName: 'B' },
    { type: 'ApexClass', fullName: 'A' },
]);
