...
try {
    ...
    addSession('uat@x.com', {
        entries: [{ type: 'ApexTrigger', fullName: 'T' }],
        targetOrg: 'prod',
        testLevel: 'NoTestRun',
    });
    ...
    ok(
        'stores target + testLevel + savedAt',
        list[0].targetOrg === 'prod' && !!list[0].testLevel && !!list[0].savedAt,
    );

    // cap at 20
    for (let i = 0; i < 25; i += 1)
        addSession('uat@x.com', { entries: [{ type: 'ApexClass', fullName: `C${i}` }] });
