...
program
    ...
    .option(
        '-w, --wait <min>',
        'minutes to wait for retrieve/deploy to finish',
        (v) => parseInt(v, 10),
    ...
    .action(async (cmdOpts) => {
        ...
        if (cmdOpts.demo) {
            ...
            if (!source) {
                ...
                stop();
                if (orgs.length === 0) {
                    console.error('No authenticated orgs. Run `sf org login web`.');
                    process.exit(1);
                }
            ...
            prepare = async (step) => {
                ...
                const types = (await describeTypes(conn, apiVersion)).filter(
                    (t) => !FOLDER_TYPES.has(t.name),
                );
                ...
                const loadComponents = async (type, { refresh = false } = {}) => {
                    const { rows, fetchedAt } = await listComponents(conn, type, {
                        apiVersion,
                        orgKey: username,
                        refresh,
                    });
                ...
                if (prefs.lastType && types.some((t) => t.name === prefs.lastType))
                    store.activeType = prefs.lastType;
        ...
        if (cmdOpts.import) {
            try {
                ...
                seedNote =
                    `Imported ${entries.length} component(s) from ${path.basename(cmdOpts.import)}` +
        ...
        try {
            ...
            setPrefs(orgKey(), {
                lastTarget: result.targetOrg || store.targetOrg,
                lastType: store.activeType,
            });
        } catch {
        ...
        if (cmdOpts.demo) {
            ...
            console.log(
                `  3. sf project deploy ${result.action === 'validate' ? 'validate' : 'start'}  -> ${result.targetOrg}  test-level ${result.testLevel}`,
            );
            process.exit(0);
        }
        ...
        console.log(
            step(1, 2, `Retrieving ${result.entries.length} components from ${c.yellow(store.sourceOrg)} …`),
        );
        const r = await retrieveFromSource({
            ...
            entries: result.entries,
            apiVersion,
            refetch: cmdOpts.refetch,
            wait: cmdOpts.wait,
        });
        if (r.reused)
            console.log(
                c.gray('      ↳ reused the cached zip (selection unchanged) — pass --refetch to re-pull'),
            );
        const { appendLog } = await import('../src/history.js');
        const logRun = (ok, code) =>
            appendLog({
                action: result.action,
                source: store.sourceOrg,
                target: result.targetOrg,
        ...
        if (r.code !== 0) {
            ...
            console.error(
                c.gray(`Your selection is saved — run "ferry ui" again to retry (it'll be pre-checked).`),
            );
            process.exit(r.code);
        }
...
program
    ...
    .action(async () => {
        ...
        for (const x of s.sessions)
            console.log(
                `  ${x.org.padEnd(38)} ${String(x.count).padStart(2)} saved · newest ${ago(x.newest)}`,
            );
        console.log(`\n${c.cyan('Metadata cache')}${c.gray('  (press r in the UI to refresh a type)')}`);
        if (!s.cache.length) console.log('  (none)');
        for (const x of s.cache)
            console.log(
                `  ${x.org.padEnd(38)} ${String(x.types).padStart(3)} types · newest ${ago(x.newest)}`,
            );
...
program
    ...
    .action(async (cmdOpts) => {
        ...
        const targets = cmdOpts.all ? [home] : [path.join(home, 'cache'), path.join(home, 'retrieve')];
        ...
        if (!cmdOpts.yes) {
            ...
            const go = await confirm({
                message: cmdOpts.all
                    ? 'Remove ALL ferry state, including saved sessions?'
                    : 'Remove cache + retrieve zips?',
                default: false,
            });
...
program
    ...
    .option(
        '-w, --wait <min>',
        'minutes to wait for retrieve/deploy',
        (v) => parseInt(v, 10),
        DEFAULT_WAIT_MINUTES,
    )
    .option('--refetch', 're-retrieve from the source org even if a matching zip is cached')
    ...
    .action(async (cmdOpts) => {
        ...
        if (cmdOpts.import) {
            ...
            try {
                ...
                if (res.wildcards.length && !asJson) {
                    console.error(
                        `Note: skipped ${res.wildcards.length} wildcard '*' type(s): ${res.wildcards.join(', ')}`,
                    );
                }
            } catch (e) {
                fail(`Could not import ${cmdOpts.import}: ${e.message}`);
            }
        } else {
        ...
        const tests = (cmdOpts.tests || '')
            .split(',')
            .map((t) => t.trim())
            .filter(Boolean);
        if (testLevel === 'RunSpecifiedTests' && tests.length === 0)
            fail('RunSpecifiedTests requires --tests <classes>.');
        ...
        try {
            ...
            if (r.code !== 0) {
                record(false, r.code);
                fail(r.error || 'Retrieve failed.', r.code);
            }

            if (!asJson)
                console.log(
                    `${action === 'validate' ? 'Validating' : 'Deploying'} to ${cmdOpts.target} (${testLevel}) …`,
                );
            code = await deployToTarget({
                retrieveDir,
                targetOrg: cmdOpts.target,
                testLevel,
                tests,
...
program
    ...
    .action(async (cmdOpts) => {
        ...
        if (!list.length) {
            console.log('No deploys logged yet.');
            return;
        }
        ...
        for (const e of list) {
            ...
            console.log(
                `  ${mark} ${verb} ${String(e.count).padStart(3)} comp  ${route.padEnd(38)} ${(e.testLevel || '').padEnd(18)} ${ago(e.at)}${e.mode === 'ci' ? c.gray(' [ci]') : ''}${took}`,
            );
        }
    });
