...
export function configFile() { return path.join(ferryHome(), 'config.json'); }
export function prefsFile() { return path.join(ferryHome(), 'prefs.json'); }
export function logFile() { return path.join(ferryHome(), 'log.json'); }
export function cacheDir(orgKey) { return path.join(ferryHome(), 'cache', safe(orgKey)); }
export function cacheFile(orgKey, type) { return path.join(cacheDir(orgKey), `${safe(type)}.json`); }
export function sessionsFile(orgKey) { return path.join(ferryHome(), 'sessions', `${safe(orgKey)}.json`); }
export function retrieveDir(orgKey) { return path.join(ferryHome(), 'retrieve', safe(orgKey)); }
