...
export function buildOrgDeployArgs({
    zipPath,
    targetOrg,
    testLevel,
    tests = [],
    validate = false,
    wait = DEFAULT_WAIT_MINUTES,
}) {
    ...
    const args = [
        ...
        sub,
        '--metadata-dir',
        zipPath, // a zip with an unpackaged/ wrapper — NOT --single-package
        '--target-org',
        targetOrg,
        '--test-level',
        testLevel,
...
export async function retrieveFromSource({
    ...
    if (code === 0 && !existsSync(zip)) {
        return {
            ...
            reused: false,
            error:
                'Nothing was retrieved from the source org — the selected components were not found there. ' +
