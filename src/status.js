  ...
  if (existsSync(cdir)) {
    for (const org of readdirSync(cdir)) {
      try {
        ...
        out.cache.push({ org, types: files.length, newest: newest ? new Date(newest).toISOString() : null });
      } catch { /* skip */ }
    }
  }
  ...
  if (existsSync(rdir)) {
    for (const org of readdirSync(rdir)) {
      ...
      if (existsSync(zip)) {
        ...
        out.retrieve.push({ org, sizeKb: Math.round(st.size / 1024), at: new Date(st.mtimeMs).toISOString() });
      }
    }
  }
  return out;
